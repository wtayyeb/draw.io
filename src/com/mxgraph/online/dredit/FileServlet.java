...
public class FileServlet extends DrEditServlet
{
			...
			throws IOException
	{
		...
		if (req.getParameter("status") != null)
		{
			...
			try
			{
				CredentialMediator credentialMediator = getCredentialMediator(
						req, resp);
				String userId = (String) req.getSession().getAttribute(
						CredentialMediator.USER_ID_KEY);

				if (userId != null
						&& credentialMediator != null
						&& credentialMediator.getStoredCredential(userId) != null)
				{
					isValid = true;
				}
			...
			try
			{
				...
				if (file != null)
				{
					...
					resp.getWriter().print(
							new ClientFile(file, content).toJson());
				}
				else
				{
		...
		try
		{
			...
			if (!clientFile.content.equals(""))
			{
				...
				file = service
						...
						.insert(file,
								ByteArrayContent
										.fromString(clientFile.mimeType,
			...
			throws IOException
	{
		...
		try
		{
			return new Scanner(response.getContent(), "UTF-8").useDelimiter(
					"\\A").next();
		}
